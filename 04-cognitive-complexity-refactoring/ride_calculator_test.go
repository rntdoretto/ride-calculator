...
func TestExecute(t *testing.T) {
	...
	for _, tt := range testTable {
		t.Run(tt.name, func(t *testing.T) {
			segments := []cognitivecomplexityrefactoring.Segment{}
			for _, i := range tt.input {
				segment := cognitivecomplexityrefactoring.Segment{i.distance, i.dateTime}
				segments = append(segments, segment)
			}
			fare := cognitivecomplexityrefactoring.CalculateRide(segments)
